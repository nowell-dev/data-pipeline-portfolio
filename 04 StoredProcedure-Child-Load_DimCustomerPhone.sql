...
Test script		:
	SELECT 'BEFORE', * FROM DimCustomerPhone ORDER BY CustomerId, EffectiveLoadBatchId
	...
	EXEC dbo.[Load_DimCustomerPhone] @LoadBatchId, 'C:\GitHub\data-pipeline-portfolio\customers_file01.csv'

	SELECT 'AFTER', * FROM DimCustomerPhone ORDER BY CustomerId, EffectiveLoadBatchId
--------------------------------------------------------------------------
*/
USE [CustomerETL]
GO
...
CREATE OR ALTER PROCEDURE dbo.[Load_DimCustomerPhone]
	@LoadBatchId INT = NULL,
...
BEGIN
	...
	DECLARE @sql NVARCHAR(MAX) = N'
		INSERT INTO #Temp ( CustomerId, Phone )
		SELECT DISTINCT
			cast( r.customer_id AS int ),
			cast( r.phone AS nvarchar(250) )
		FROM OPENROWSET(
	...
	WHEN	MATCHED
	AND		isnull( raw.Phone, '' ) <> isnull( dest.Phone, '' )
	...
	INSERT INTO DimCustomerPhone	(	CustomerId, 
										Phone, 
										EffectiveLoadBatchId,
										ExpirationLoadBatchId
									)
